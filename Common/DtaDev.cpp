...
void DtaDev::puke()
{
    ...
    /* IDENTIFY */
    cout << "\n" << dev << " " << disk_info.devType << " "<< disk_info.modelNum << " "
         << disk_info.firmwareRev << " " << disk_info.serialNum << std::endl;
    /* TPer */
    auto y_or_n = [](uint8_t a){return a ? "Y, " : "N, ";};
    auto n_or_y = [](uint8_t a){return a ? "N, " : "Y, "; };
    if (disk_info.TPer) {
        cout << "TPer function (" << HEXON(4) << FC_TPER << HEXOFF << ")" << std::endl;
        cout << "    ACKNAK = " << y_or_n(disk_info.TPer_ACKNACK)
             << "ASYNC = " << y_or_n(disk_info.TPer_async)
             << "BufferManagement = " << y_or_n(disk_info.TPer_bufferMgt)
             << "comIDManagement  = " << y_or_n(disk_info.TPer_comIDMgt)
             << "Streaming = " << y_or_n(disk_info.TPer_streaming)
             << "SYNC = " << y_or_n(disk_info.TPer_sync)
             << std::endl;
    }
    if (disk_info.Locking) {

        cout << "Locking function (" << HEXON(4) << FC_LOCKING << HEXOFF << ")" << std::endl;
        cout << "    Locked = " << y_or_n(disk_info.Locking_locked)
             << "LockingEnabled = " << y_or_n(disk_info.Locking_lockingEnabled)
             << "LockingSupported = " << y_or_n(disk_info.Locking_lockingSupported);
        cout << "MBRDone = " << y_or_n(disk_info.Locking_MBRDone)
             << "MBREnabled = " <<y_or_n(disk_info.Locking_MBREnabled)
             << "MediaEncrypt = " << y_or_n(disk_info.Locking_mediaEncrypt)
             << std::endl;
    }
    if (disk_info.Geometry) {
        auto geo_bytes{disk_info.Geometry_alignmentGranularity *
                       disk_info.Geometry_logicalBlockSize};

        cout << "Geometry function (" << HEXON(4) << FC_GEOMETRY << HEXOFF << ")\n"
             << "    Align = " << y_or_n(disk_info.Geometry_align)
    ...
    if (disk_info.Enterprise) {
        ...
        cout << "    Range crossing = " << n_or_y(disk_info.Enterprise_rangeCrossing)
    ...
    if (disk_info.SingleUser) {
        ...
        cout << "    ALL = " << y_or_n(disk_info.SingleUser_all)
             << "ANY = " << y_or_n(disk_info.SingleUser_any)
             << "Policy = " << y_or_n(disk_info.SingleUser_policy)
    ...
    if (disk_info.Unknown)
        cout << "**** " << (uint16_t)disk_info.Unknown << " **** Unknown function codes IGNORED "
             << std::endl;
}
