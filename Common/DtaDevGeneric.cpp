...
DtaDevGeneric::DtaDevGeneric(const std::string& devref)
{
	DtaDevOS::init(devref.c_str());
}
...
uint8NOCODE(saveDS, uint8_t index, uint32_t offset, uint32_t length, char * password, char * filename)
